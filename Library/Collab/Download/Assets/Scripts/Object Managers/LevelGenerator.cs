...
public class LevelGenerator : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < distanceToEnd; i++)
        {
            ...
            while (Physics2D.OverlapCircle(generatorPoint.position, .2f, whatIsRoom))
            {
                ...
                //while (wantedDirection == selectedDirection || wantedDirection == (Direction)(((int)firstDirection + 2) % 4))
                    //wantedDirection = (Direction)Random.Range(0, 4);
        ...
        CreateRoomOutline(Vector3.zero, false, 0);

        foreach(GameObject room in rooms)
        {
            CreateRoomOutline(room.transform.position, true, -1);
        }
        CreateRoomOutline(endRoom.transform.position, false, 0);
    ...
    public void CreateRoomOutline(Vector3 roomPosition, bool shouldHaveCenter, int size)
    {
        ...
        generatedOutlines[generatedOutlines.Count - 1].GetComponent<Room>().sizeMultiplier = Mathf.Clamp((size + 1) / 2f, .75f, 1.5f);

        if (shouldHaveCenter)
        {
            Instantiate(centers[size][Random.Range(1, centers[size].Length)], roomPosition, Quaternion.identity);
        }
        else
        {
