...
public class BugEnemy : Enemy
{
    ...
    void Start()
    {
        ...
        //attackRange = Random.Range(attackRange - 2f, attackRange + 1f);
    }
    ...
    public IEnumerator Shoot()
    {
        ...
        if(isGreen)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                Bullet bullet = Instantiate(bulletPrefab, firePoint.transform.position, Quaternion.Euler(firePoint.rotation.eulerAngles + deviation));
                bullet.whoShotIt = this.transform;
